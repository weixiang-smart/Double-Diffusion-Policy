...
import ddp.utils.utils as utils
from pyinstrument import Profiler
from ddp.utils.logger import Logger
import gc
...
def main(cfg):
    ...
    # create model
    high_model = instantiate(cfg.algo.policy,
                        shape_meta=cfg.task.shape_meta)
    high_model.to(device)
    high_model.eval()

    # start training
    optimizers = high_model.get_optimizers()
    schedulers = high_model.get_schedulers(optimizers)



    # robot diffusion policy
    low_model = instantiate(cfg.algo.low_policy,
                        shape_meta=cfg.task.shape_meta)
    low_model.to(device)
    low_model.train()

    # start training
    low_model_optimizers = low_model.get_optimizers()
    low_model_schedulers = low_model.get_schedulers(low_model_optimizers)
    ...
    checkpoint_path = 'experiments/libero/LIBERO_90/diffusion_policy/final/block_32/0/stage_1/multitask_model_epoch_0100.pth'
    ...
    if checkpoint_path is not None:
        ...
        utils.soft_load_state_dict(high_model, loaded_state_dict)
    ...
    high_model.preprocess_dataset(dataset, use_tqdm=train_cfg.use_tqdm)
    train_dataloader = instantiate(
        cfg.train_dataloader, 
        dataset=dataset)
    

    for param in high_model.diffusion_model.parameters():
            param.requires_grad = False


    low_model.get_high_model(high_model)
    ...
    for epoch in range(start_epoch, train_cfg.n_epochs + 1):
        ...
        if epoch % train_cfg.save_interval == 0 and epoch > 0:
            ...
            utils.save_state({
                'model': low_model,
                'optimizers': low_model_optimizers,
                'schedulers': low_model_schedulers,
                'scaler': scaler,
                'epoch': epoch,
                'stage': cfg.stage,
                'steps': steps,
        ...
        if cfg.rollout.enabled and epoch > 0 and epoch % cfg.rollout.interval == 0:
            rollout_results = env_runner.run(low_model, n_video=cfg.rollout.n_video, do_tqdm=train_cfg.use_tqdm)
            print(
        ...
        [low_model_scheduler.step() for low_model_scheduler in low_model_schedulers]
